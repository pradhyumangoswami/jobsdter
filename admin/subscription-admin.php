...
// Subscription meta box is added directly in post-types/candidate.php
// in the jobster_add_candidate_metaboxes() function

// Render subscription meta box
if (!function_exists('jobster_subscription_metabox_render')):
    ...
    $profile_views = get_post_meta($candidate_id, 'candidate_profile_views', true) ?: 0;
            ...
            // Using the existing candidate nonce from the main form
    ?>
...
// Subscription saving is handled in the main candidate save function
// in post-types/candidate.php - jobster_candidate_meta_save()

// Add subscription column to candidates list
if (!function_exists('jobster_add_subscription_column')):
    function jobster_add_subscription_column($columns) {
        $columns['subscription'] = __('Subscription', 'jobster');
        return $columns;
    }
endif;
add_filter('manage_candidate_posts_columns', 'jobster_add_subscription_column');

// Display subscription column content
if (!function_exists('jobster_display_subscription_column')):
    function jobster_display_subscription_column($column, $post_id) {
...
add_action('manage_candidate_posts_custom_column', 'jobster_display_subscription_column', 10, 2);

// Add subscription filter to candidates list
if (!function_exists('jobster_add_subscription_filter')):
    function jobster_add_subscription_filter() {
        global $typenow;
...
add_action('restrict_manage_posts', 'jobster_add_subscription_filter');

// Filter candidates by subscription status
if (!function_exists('jobster_filter_candidates_by_subscription')):
    function jobster_filter_candidates_by_subscription($query) {
...
add_action('pre_get_posts', 'jobster_filter_candidates_by_subscription');
?>